...
class MockSendEmailToSubsriberTask:
...
class SubscriberCreationTestCase( MockSendEmailToSubsriberTask, TestCase):
...
class SubscriberManagerTestCase(TestCase):
    ...
    def testConfirmedSubscribersForMailingList(self):
        mailing_list = MailingList.objects.create(
            name = 'unit test',
            owner = get_user_model().objects.create_user(username='asdf')
        )
        confirmed_users = [
...
class ListMailingListWithAPITestCase(APITestCase):
    ...
    def test_listing_all_my_mailing_lists(self):
        ...
        response = self.client.get(reverse('mailinglist:api-mailing-list-list'))
